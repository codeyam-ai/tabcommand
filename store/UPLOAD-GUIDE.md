# TabCommand — Chrome Web Store upload guide (Public listing)
...
|---|---|---|
| `tabcommand-<version>.zip` | The packaged extension (manifest at the zip root). Attached to the matching GitHub Release by `release.yml`; the local copy is gitignored | Package → Upload new package |
...
2. Click **"Add new item"** and upload **`tabcommand-<version>.zip`** — download it
   from the GitHub Release for the matching tag, or build it locally.
3. **Store listing** tab:
